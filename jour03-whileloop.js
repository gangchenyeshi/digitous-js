...
var count=0;
 ...
 var max=6;
 while(dice<6) {
     dice=Math.floor(Math.random() * (max - min + 1));
     count++;
...
while( i < 100) {
    ...
    //console.log(`First Random no : ${firstRandom}`);
    var secondRandom=Math.floor(Math.random() * (max - min +1));
    //console.log(`Second Random no : ${secondRandom}`);
    i++;
    ussainBold = ussainBold + firstRandom;
    tysonGay = tysonGay + secondRandom;
     if(ussainBold >=100) {
        console.log(`ussainBold  is winner and total : ${ussainBold}`);
     } else if(tysonGay >= 100) {
        console.log(`tysonGay  is winner and total : ${tysonGay}`); 
     }
}
